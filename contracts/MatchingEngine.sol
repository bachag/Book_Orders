    ...
    EventfulMatchingEngine
{
    ...
    function _buyAmountLessThanRatio(
        ...
        if (sellingToken1 == 1) {
            // Search through bids (look through orders that want to buy token1)
            ...
            while (
                ...
                if (biggerToken == 1) {
                    // The amount of token1 is > the amount of token2
                    if (orders[curr.id].biggerToken == 1) {
                        // The best bid also has a token1Amt > token2Amt
                        if (priceRatio >= orders[curr.id].priceRatio) {
                            ...
                            token2Amt = uint128(
                                (uint256(token1Amt) * 1_000_000_000_000_000) /
                                    priceRatio
                            );
                            uint128 buyAmt = token2Amt <
                                orders[curr.id].sellingTokenAmt
                                ? token2Amt
                                : orders[curr.id].sellingTokenAmt;
                            if (_buy(curr.id, buyAmt)) {
                                // If the order was completely filled, remove it
                                bids[bids[curr.id].next].prev = bids[curr.id]
                                    .prev;
                                bids[bids[curr.id].prev].next = bids[curr.id]
                                    .next;
                                delete bids[curr.id];
                            }
                            token1Amt -= uint128(
                                (uint256(buyAmt) * orders[curr.id].priceRatio) /
                                    1_000_000_000_000_000
                            ); // Potentially inefficient and maybe dangerous;
                        } else {
                            break;
                        }
                    } else {
                        ...
                        token1Amt -= uint128(
                            (uint256(buyAmt) * 1_000_000_000_000_000) /
                                orders[curr.id].priceRatio
                        ); // Potentially inefficient and maybe dangerous;
                    }
                } else {
                    // biggerToken == 2
                    if (orders[curr.id].biggerToken == 2) {
                        ...
                        // biggerToken == 2 && orders[curr.id].biggerToken == 1
                        break;
                    }
                }
                // Get next best bid
            ...
            // sellingToken1 == 0; Search through asks
                ...
                if (biggerToken == 1) {
                    if (orders[curr.id].biggerToken == 1) {
                        if (priceRatio <= orders[curr.id].priceRatio) {
                            ...
                            token1Amt = uint128(
                                (uint256(token2Amt) * priceRatio) /
                                    1_000_000_000_000_000
                            );
                            uint128 buyAmt = token1Amt <
                                orders[curr.id].sellingTokenAmt
                                ? token1Amt
                                : orders[curr.id].sellingTokenAmt;
                            if (_buy(curr.id, buyAmt)) {
                                // If the order was completely filled, remove it
                                asks[asks[curr.id].next].prev = asks[curr.id]
                                    .prev;
                                asks[asks[curr.id].prev].next = asks[curr.id]
                                    .next;
                                delete asks[curr.id];
                            }
                            token2Amt -= uint128(
                    ...
                    if (orders[curr.id].biggerToken == 2) {
                        if (priceRatio >= orders[curr.id].priceRatio) {
                            ...
                            token1Amt = uint128(
                                (uint256(token2Amt) * 1_000_000_000_000_000) /
                                    priceRatio
                            );
                            uint128 buyAmt = token1Amt <
                                orders[curr.id].sellingTokenAmt
    ...
    function makerOrder(
        ...
        if (sellingToken1 == 1) {
            // A higher ratio of token2 may have been spent in _buyAmountLessThanRatio()
            if (token1Amt == 0) return; // Function is exited if entire order was executed as a taker order
            token2Amt = biggerToken == 1
                ? uint128(
                    (uint256(token1Amt) * 1_000_000_000_000_000) / priceRatio
                )
                : uint128(
                    (uint256(token1Amt) * priceRatio) / 1_000_000_000_000_000
                );
        } else {
            // A higher ratio of token1 may have been spent in _buyAmountLessThanRatio()
            if (token2Amt == 0) return; // Function is exited if entire order was executed as a taker order
            token1Amt = biggerToken == 1
                ? uint128(
                    (uint256(token2Amt) * priceRatio) / 1_000_000_000_000_000
                )
                : uint128(
                    (uint256(token2Amt) * 1_000_000_000_000_000) / priceRatio
                );
        }
        ...
        if (sellingToken1 == 1) {
            // If selling token 1, then add to asks
            if (dllPosition == 1 || asks[dllPosition].prev == 0) {
                // Try and add ask to the front
                if (orders[asks[0].next].biggerToken == 1 && biggerToken == 1) {
                    if (priceRatio <= orders[asks[0].next].priceRatio) {
                        // Proposed position wrong, find correct position
                        dllPosition = _findInsertPosition(
                            priceRatio,
                            biggerToken,
                            1
                        ); // 1 for ask
                    ...
                    orders[asks[0].next].biggerToken == 2 && biggerToken == 2
                ) {
                    if (priceRatio >= orders[asks[0].next].priceRatio) {
                        // if ratio >= curr ratio its a worse order
                        dllPosition = _findInsertPosition(
                            priceRatio,
                            biggerToken,
                            1
                    ...
                    orders[asks[0].next].biggerToken == 1 && biggerToken == 2
                ) {
                    // Always a worse order
                    dllPosition = _findInsertPosition(
                        priceRatio,
                        biggerToken,
                        1
                    ...
                    // orders[asks[0].id].biggerToken == 2 && biggerToken == 1
                    // Proposed position is correct, insert at front
                    _insertFirstOrder(id, 1); // 1 for ask
                    emit MakerOrderCreated(id, 1);
                    return;
                }
            } else if (dllPosition == 0) {
                // Order is proposed to be added to back of orderbook (worst order)
                if (orders[asks[0].prev].biggerToken == 1 && biggerToken == 1) {
                    if (priceRatio > orders[asks[0].prev].priceRatio) {
                        // The order is better than the current worst order
                        dllPosition = _findInsertPosition(
                            priceRatio,
                            biggerToken,
                            1
                    ...
                    orders[asks[0].prev].biggerToken == 2 && biggerToken == 2
                ) {
                    if (priceRatio < orders[asks[0].prev].priceRatio) {
                        dllPosition = _findInsertPosition(
                            priceRatio,
                    ...
                    orders[asks[0].prev].biggerToken == 1 && biggerToken == 2
                ) {
                    dllPosition = _findInsertPosition(
                        priceRatio,
                        biggerToken,
                ...
                // Want worse than previous and better than current id
                if (
                    orders[asks[dllPosition].prev].biggerToken == 1 &&
                    biggerToken == 1
                ) {
                    if (
                        priceRatio > orders[asks[dllPosition].prev].priceRatio
                    ) {
                        // If its > prev ratio, its a better order than prev
                        dllPosition = _findInsertPosition(
                            priceRatio,
                            biggerToken,
                            1
                    ...
                    if (orders[dllPosition].biggerToken == 1) {
                        // If curr biggerToken is 2, the order is properly placed
                        if (priceRatio <= orders[dllPosition].priceRatio) {
                            // If its <= curr ratio, it's worse than curr
                            dllPosition = _findInsertPosition(
                                priceRatio,
                                biggerToken,
                                1
                    ...
                    orders[asks[dllPosition].prev].biggerToken == 2 &&
                    biggerToken == 2
                ) {
                    if (
                        priceRatio < orders[asks[dllPosition].prev].priceRatio
                    ) {
                        // if ratio < prev ratio, its a better order
                        dllPosition = _findInsertPosition(
                            priceRatio,
                            biggerToken,
                            1
                    ...
                    // orders[asks[dllPosition].prev].biggerToken == 2 && biggerToken == 1
            ...
            // Selling token 2; bids
            if (dllPosition == 1 || bids[dllPosition].prev == 0) {
                // Try and add bid to the front
                if (orders[bids[0].next].biggerToken == 1 && biggerToken == 1) {
                    ...
                    orders[bids[0].next].biggerToken == 2 && biggerToken == 2
                ) {
                    if (priceRatio <= orders[bids[0].next].priceRatio) {
                        dllPosition = _findInsertPosition(
                            priceRatio,
                    ...
                    orders[bids[0].next].biggerToken == 2 && biggerToken == 1
                ) {
                    // Always a worse order
                    dllPosition = _findInsertPosition(
                        priceRatio,
                    ...
                    // orders[bids[0].id].biggerToken == 1 && biggerToken == 2
                    // Proposed position is correct, insert at front
                    _insertFirstOrder(id, 0); // 0 for bid
                    emit MakerOrderCreated(id, 1);
                    return;
                ...
                if (orders[bids[0].prev].biggerToken == 1 && biggerToken == 1) {
                    ...
                    orders[bids[0].prev].biggerToken == 2 && biggerToken == 2
                ) {
                    if (priceRatio > orders[bids[0].prev].priceRatio) {
                        dllPosition = _findInsertPosition(
                            priceRatio,
                    ...
                    orders[bids[0].prev].biggerToken == 2 && biggerToken == 1
                ) {
                    dllPosition = _findInsertPosition(
                        priceRatio,
                        biggerToken,
                ...
                // Want worse than previous and better than current id
                if (
                    orders[bids[dllPosition].prev].biggerToken == 1 &&
                    biggerToken == 1
                ) {
                    if (
                        priceRatio < orders[bids[dllPosition].prev].priceRatio
                    ) {
                        dllPosition = _findInsertPosition(
                            priceRatio,
                            biggerToken,
                    ...
                    orders[bids[dllPosition].prev].biggerToken == 2 &&
                    biggerToken == 2
                ) {
                    if (
                        priceRatio > orders[bids[dllPosition].prev].priceRatio
                    ...
                    // orders[bids[dllPosition].prev].biggerToken == 1 && biggerToken == 2
                    // The proposed order is a better order than the previous order
                    dllPosition = _findInsertPosition(
                        priceRatio,
                        biggerToken,
    ...
    function _findInsertPosition(
        ...
        if (orderType == 0) {
            // loop through bids and find insert position
            Node memory curr = bids[bids[0].next];
            while (curr.id != 0) {
                if (orders[curr.id].biggerToken == 1 && biggerToken == 1) {
                    ...
                    orders[curr.id].biggerToken == 2 && biggerToken == 2
                ) {
                    if (priceRatio > orders[curr.id].priceRatio) {
                        return curr.id;
                    }
                } else if (
                    orders[curr.id].biggerToken == 1 && biggerToken == 2
                ) {
                    return curr.id;
                }
            ...
            // loop through asks and find insert position
            Node memory curr = asks[asks[0].next];
            while (curr.id != 0) {
                if (orders[curr.id].biggerToken == 1 && biggerToken == 1) {
                    if (priceRatio > orders[curr.id].priceRatio) {
                    ...
                    orders[curr.id].biggerToken == 2 && biggerToken == 1
                ) {
                    return curr.id;
                }
